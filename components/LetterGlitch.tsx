...
const LetterGlitch = ({
  ...
  const drawLetters = () => {
    ...
    letters.current.forEach((letter, index) => {
      ...
      ctx.fillText(letter.char, (index % grid.current.columns) * charWidth, Math.floor(index / grid.current.columns) * charHeight);
    });
  };
  ...
  const containerStyle: CSSProperties = { position: "relative", width: "100%", height: "100%", backgroundColor: "#000000", overflow: "hidden" };
  const canvasStyle: CSSProperties = { display: "block", width: "100%", height: "100%" };
  const vignetteBase: CSSProperties = { position: "absolute", top: 0, left: 0, width: "100%", height: "100%", pointerEvents: "none" };
  ...
  return (
    ...
    >
      ...
      {outerVignette && (
        <motion.div
          style={{ ...vignetteBase, background: "radial-gradient(circle, rgba(0,0,0,0) 60%, rgba(0,0,0,1) 100%)" }}
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
      ...
      {centerVignette && (
        <motion.div
          style={{ ...vignetteBase, background: "radial-gradient(circle, rgba(0,0,0,0.8) 0%, rgba(0,0,0,0) 60%)" }}
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ delay: 0.5, duration: 1, ease: "easeOut" }}
        />
